...
class FakeAsyncRuntime:
    ...
    async def async_run(self, *inputs):
        self.owner.async_calls.append(inputs)
...
class FakeRebel:
    ...
    def AsyncRuntime(self, path, **kwargs):
        ...
        instance = FakeAsyncRuntime(self)
        self.async_instances.append(instance)
        return instance
...
def _load_with_fake(monkeypatch, fake_rebel, compiled_model, **runtime_options):
    monkeypatch.setattr(
...
def loaded_runtime(tmp_path, monkeypatch, fake_rebel):
    return _load_with_fake(
        monkeypatch,
        fake_rebel,
        _compiled_model(tmp_path / "loaded.rbln"),
