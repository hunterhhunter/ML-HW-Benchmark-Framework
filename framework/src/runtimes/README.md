...
Mobilint adapter의 SDK 실행 slot과 framework callback job은 수명이 다릅니다. SDK slot은 `Future.get()`이 terminal이 되고 raw output 정규화가 끝난 직후 반환합니다. `_jobs`와 input 참조는 framework callback이 완전히 반환될 때까지 유지하므로, 다음 SDK 요청은 callback 반환 지연과 무관하게 제출할 수 있지만 shutdown과 model dispose는 callback/waiter 종료 전에는 완료되지 않습니다. Slot 반환은 job별 exact-once 상태로 보호하며, pending Future가 있는 동안에는 기존의 nonblocking capacity 제한을 유지합니다.
...
- native async를 1,000건과 3,000건 연속 실행해 `submitted == accepted == completed`, `failed == timed_out == outstanding == 0`을 확인하고, 다음 추론 중에도 보관한 직전 output의 hash가 변하지 않는지 검증합니다. 수정 전후 QPS와 p95/p99도 함께 기록합니다.
