...
def run_auto_prepare(profile: dict, args: argparse.Namespace, target=None):
    ...
    can_auto_prepare_model = (
        ...
        and getattr(target, "target_id", None) != "furiosa-rngd-torch"
...
def _validate_furiosa_torch_cli(
    ...
    if getattr(args, "compile", True) is False:
        raise ValueError(
            ...
            "runtime must compile a strict static graph for RNGD."
        )

    from runtimes.furiosa_torch_models import get_torch_model_adapter

    try:
        adapter = get_torch_model_adapter(args.model)
    ...
    try:
        ...
        raise ValueError(
            "furiosa-rngd-torch requires an existing local model directory."
        ) from exc
    if not model_path.is_dir():
        raise ValueError(
            "furiosa-rngd-torch requires an existing local model directory: "
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--target", type=str, default=None, help="실행 target_id (예: cpu, cuda, furiosa-rngd, furiosa-rngd-torch, mobilint-aries, mobilint-regulus, hailo8, rbln-static, rbln-vllm). 지정 시 backend/device보다 우선합니다.")
    parser.add_argument("--backend", type=str, default="onnxruntime", choices=["onnxruntime", "iree", "vllm", "hailort", "deepx", "furiosa_llm", "furiosa", "rngd", "furiosa_torch", "rbln", "rbln_vllm"], help="추론을 실행할 백엔드 (기본: onnxruntime)")
...
def main():
    ...
    # 생성 런타임은 default_model_path, Furiosa Torch는 검증된 로컬 BERT를 사용한다.
